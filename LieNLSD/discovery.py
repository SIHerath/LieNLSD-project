...
def sphere_basis_terms(x, y, z, sphere_basis, device):
    if sphere_basis == 'linear':
        return torch.tensor([1, x, y, z], device=device)
    if sphere_basis == 'quadratic':
        return torch.tensor([1, x, y, z, x ** 2, y ** 2, z ** 2, x * y, x * z, y * z], device=device)
    raise ValueError(f'Unknown sphere basis: {sphere_basis}')


def sphere_basis_dim(sphere_basis):
    if sphere_basis == 'linear':
        return 4
    if sphere_basis == 'quadratic':
        return 10
...
def compute_theta(data, task, device, sphere_basis='quadratic'):
    if task == 'top':
        theta = {}
        theta['th'] = data['x'][0]
        ...
        theta = {}
        theta['th'] = sphere_basis_terms(x, y, z, sphere_basis, device)
...
def compute_theta_n(data, task, device, sphere_basis='quadratic'):
    theta = compute_theta(data, task, device, sphere_basis=sphere_basis)
    if task == 'top':
        th = theta['th']
        ...
        th = theta['th']
        basis_dim = sphere_basis_dim(sphere_basis)
        theta_n = torch.zeros((3, 3 * basis_dim), device=device)
        theta_n[0, :basis_dim] = th
        theta_n[1, basis_dim:2 * basis_dim] = th
        theta_n[2, 2 * basis_dim:] = th
    elif task in ['heat', 'burger']:
...
def symmetry_discovery(model, train_loader, task, device, threshold, sample, sphere_basis='quadratic', **kwargs):
    model.eval()
    with torch.no_grad():
        num_discovery_samples = kwargs.get('num_discovery_samples', 100)
        if task == 'top':
            CTC = torch.zeros((16, 16), device=device)
        elif task in ['sphere', 'sphere_truth', 'sphere_lse', 'cylinder']:
            sphere_dim = 3 * sphere_basis_dim(sphere_basis)
            CTC = torch.zeros((sphere_dim, sphere_dim), device=device)
        ...
        for j, data in enumerate(train_loader):
            if num_discovery_samples != -1:
                start = sample * num_discovery_samples
                end = (sample + 1) * num_discovery_samples
                if j < start:
                    continue
                elif j >= end:
                    break
            ...
            if task == 'top':
                ...
                feature = torch.stack([data['x'], data['y'], data['z']], dim=1)
            elif task in ['heat', 'burger']:
            ...
            theta_n = compute_theta_n(data, task, device, sphere_basis=sphere_basis)
            if task == 'top':
            ...
            Ci = df @ theta_n
            CTC += Ci.T @ Ci
        # MPS does not support float64 SVD, so run the decomposition on CPU
        # in double precision and then move the result back to the active device.
        ctc_cpu64 = CTC.detach().to('cpu').to(torch.float64)
        _, S2, Vh = torch.linalg.svd(ctc_cpu64)
        S2 = S2.to(device=device, dtype=torch.float32)
