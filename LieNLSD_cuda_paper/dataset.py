...
def get_dataset(args):
    if args['task'] == 'top':
        ...
        train_dataset = SphereDataset(mode='train')
        val_dataset = SphereDataset(mode='val')
        args['input_dim'] = 3
        args['output_dim'] = 1
        args['classify'] = False
...
class SphereDataset(Dataset):
    def __init__(self, path=f'{data_path}', mode='train'):
        super().__init__()
        sphere_name = 'surface_sphere'
        try:
            print(f'Loading existing {sphere_name} {mode} data...')
            query = torch.load(f'{path}/{sphere_name}/{mode}-query.pt', weights_only=True)
            sdf = torch.load(f'{path}/{sphere_name}/{mode}-sdf.pt', weights_only=True)
